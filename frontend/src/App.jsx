...
// Player-tier probability distribution from the multi-feature Random Forest
// proximity engine (draft_risk_model). 6 NBA tiers, peak_wa distribution,
// outlier-risk indicator. Reads p.riskProfile.tierProbs + .pwDistribution +
// .outlierRisk (populated by inject_draft_risk.py from Sprint-5.5.E CSV).
// ═══════════════════════════════════════════════════════════
function RfTierForecast({p}) {
  ...
  return (
    <div className="rounded-2xl p-5" style={{background:"#0d1117",border:`1px solid ${modal.color}55`}}>
      ...
      {/* Outlier-risk disclaimer */}
      {rp.outlierRisk === "high" && (
        <div className="mt-4 text-xs italic" style={{color:"#9ca3af", lineHeight:1.4}}>
          High outlier-risk: thin comp pool or wide outcome spread. Modal tier
          is most likely but uncertainty is elevated — comp-based forecasts
          cannot capture true outlier stars or injury-driven busts. Trust
          scout eye-test more.
        </div>
      )}
    </div>
    ...
    <div className="space-y-5">
      ...
      {p.functionalSize && (() => {
        ...
        const defensiveVerdict = (side) => {
          const dh = side.height_delta;
          const dws = side.wingspan_delta;
          const avg = [dh, dws].filter(x => x != null).reduce((a, b) => a + b, 0) 