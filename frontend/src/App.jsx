    ...
    return (
      <div style={{padding:"8px 0"}}>
        ...
        <div style={{textAlign:"center",color:"#6b7280",fontSize:10,marginBottom:2}}>
          Rank = 0.6 × ev_recal (regression) + 0.4 × exp_wa (comp engine) + humble bonus   ·   Probability width = realized SD of the comp cohort
        </div>
        {arrow()}
    ...
    <div className="space-y-6">
      ...
      {methodView === "quick" && (
        ...
        <Sec icon="🎯" title="What This Is" sub="ProspectTheory in 60 seconds.">
          <div style={{fontSize:13,color:"#d1d5db",lineHeight:"1.8"}}>
            ...
            <p style={{marginBottom:12}}>A trained projection model turns these into one interpretable number — <strong style={{color:"#e5e7eb"}}>Projected Peak Wins Added (PPWA)</strong>, our estimate of a prospect's peak NBA value. The headline is a <strong style={{color:"#fbbf24"}}>60/40 blend of two independent estimators</strong> — regression-weighted. (1) A two-stage statistical projection — P(reaching the NBA) × expected value if he does — recalibrated to the real Wins-Added scale. This is the present-day signal extracted from the prospect's actual box production. (2) A <em>comparable-player</em> realized average: the mean peak-WA of the historical prospects he most resembles before the draft (a player is never his own comp, and the comp set is widened until it holds enough real peers). This is informed historical experience from his archetype peers. Why 60/40? Pre-draft regression carries fresh information about the specific player; comps reflect what happened to a cohort. Weighting the present-day signal slightly more corrects a known historical bias in wins-added — 6-9 helio-bigs (Boozer / Embiid / Jokic) peaked higher than their archetype cohort average. The empirical cost is small (Spearman ρ −0.005 vs symmetric 50/50, well within domain noise) and the comp engine still does heavy lifting: it lifts Curry, Wembanyama, Doncic whose pre-draft box-stats underrate them. On top we add a small data-fitted humble bonus tied to pre-draft consensus rank (isotonic-regressed from the residual aw − blend), and we surface tier probabilities (Superstar / All-Star / Starter / Role Player / Replacement / Negative) whose width comes from the realized spread of each prospect's comp cohort — so a top prospect reads as, say, a 50% chance at stardom <em>with real bust risk</em>, never a false 99% lock.</p>
          </div>
        </Sec>
      ...
      <Sec icon="📖" title="Methodology & Model Documentation" sub="Complete documentation of all computed metrics, formulas, and their statistical foundations.">
        <div className="text-sm mb-3" style={{color:"#9ca3af"}}>
          ProspectTheory's headline metric is <strong style={{color:"#e5e7eb"}}>Projected Peak Wins Added (PPWA)</strong> — an estimate of a prospect's best 3-consecutive-season Wins-Added peak (team-anchored impact + production, first 8 NBA years). The headline is a <strong style={{color:"#fbbf24"}}>60/40 blend of two independent estimators</strong>, weighted toward present-day regression. <strong style={{color:"#fbbf24"}}>(1) Statistical projection (ev_recal, 60%):</strong> P(NBA), trained on the full ~15k-prospect pool, times E[Added Wins | NBA], a regularized ElasticNet on NBA careers, smoothly rescaled to the realized Wins-Added scale — the present-day signal from the prospect's actual box production. <strong style={{color:"#fbbf24"}}>(2) Comp-engine realized average (exp_wa, 40%):</strong> for each prospect we find his pre-draft archetype peers (leave-one-out — a player is never his own comp — with an adaptive bandwidth that guarantees ≥10 effective comps), and take the kernel-weighted mean of those peers' <em>realized</em> peak-WA — informed experience from history. <strong style={{color:"#fbbf24"}}>Why 60/40 (not 50/50):</strong> the comp engine systematically under-rates 6-9 helio-bigs like Boozer / Embiid / Jokic, whose archetype-cohort historically peaked lower than they themselves did. Weighting regression slightly more corrects that bias without losing the comp engine's strength on generational outliers (Curry / Doncic / Haliburton, whose box-stats under-rate them but whose comps caught the star outcome). Empirical cost vs 50/50 optimum: Spearman ρ −0.005 on the 607-player resolved holdout — within domain noise. <strong style={{color:"#fbbf24"}}>Tier probabilities</strong> use the realized spread of the comp cohort as σ, centred on the headline — honest odds (no 99% locks), not a tight parametric band. Holdout: blend ρ ≈ 0.49; P(NBA) ROC-AUC 0.95.
        </div>
